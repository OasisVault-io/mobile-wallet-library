...
If you use `nvm`, install and activate the pinned version before installing dependencies:
...
If you use `volta`, the pinned `node` and `pnpm` versions in `package.json` are activated automatically from the project directory.
...
> Since the project relies on pnpm workspaces, you cannot use [`npm`](https://github.com/npm/cli) for development without manually migrating.
...
To apply lint fixes, run the following:
...
- `pnpm lint`: lint files with [Oxlint](https://oxc.rs/docs/guide/usage/linter.html).
